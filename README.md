...
**Live-parity options (T6, PR-1 plumbing + PR-2 regime + PR-3 direction_filter + F8 HTF EMA + PR-4 ADR bias):**
...
- `--with-conflict-resolver` / `--without-conflict-resolver` — port the live conflict resolver (PR-4b: helper lifted into `analytics/signal/gates.py::_apply_conflict_resolver`; backtest-replay wiring needs a runner-level cross-strategy pooling pass — flag accepted but no-op in PR-4).
