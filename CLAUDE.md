...
- `analytics/` — analytics data layer (DuckDB-backed):
  ...
  - `digest_lib.py` — pure aggregation lib over `backtest_runs`: 10 pre-canned SQL queries (`symbol`, `strategy`, `tf`, `combos`, `adr_ab`, `volume_ab`, `day_filter_ab`, `direction_bias`, `consistency`, `recovery_factor`); each returns `{"columns": [...], "rows": [...]}` for API + CLI reuse; `run_digest(conn, query, min_trades, top_n)` dispatches by name; powers `GET /api/backtest/analysis` and `buibui digest` CLI
