...
- `analytics/` — analytics data layer (DuckDB-backed):
  ...
  - `stats_lib.py` — pure stats lib: `compute_p1p2_daily`, `compute_hourly_extremes`, `compute_adr`, `compute_dow_patterns`, `compute_session_breakdown`, `compute_weekly_p1p2`, `compute_all` → `StatsBundle`; all times in MYT (UTC+8) using `+ INTERVAL 8 HOUR` offset; raises `ValueError` on empty data
...
- `signals/` — signal detection daemon package:
  ...
  - `alert_formatter.py` — `SignalEvent` + `StatsContext` dataclasses; `format_signal_alert()` → Markdown Telegram message with SL/TP levels + optional stats context line (`📐 Thu: P1=Low 58% · ADR 2.8% (43% used) · High peak ~14:00 MYT`)
