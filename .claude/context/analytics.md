...
- **N3 survivorship guard**: `symbol_lifecycle` table (PK `symbol`; `status`, `onboard_ms`, `first_checked_ms`, `last_checked_ms`, `delisted_noted_ms`); `upsert_symbol_lifecycle(conn, df)` / `get_symbol_lifecycle(conn)` in `store/market_data.py`, re-exported via `data_store`
...
- `data_fetcher.py` — pure fetch: Binance Futures API → DataFrames (no DB). `fetch_klines` accepts `KlineClient = Client | OKXClient` and branches on `isinstance(client, OKXClient)` (OKX returns a ready OHLCV DataFrame; Binance maps raw rows). `DATA_SOURCE=okx` selects the OKX adapter via `utils.binance_client.create_data_client`. N3: `fetch_futures_symbol_info(client)` → `LIFECYCLE_COLUMNS` (`symbol`, `status`, `onboard_ms`) from `futures_exchange_info()`, USDT-quoted PERPETUAL only, any status (Binance-only)
- `data_sync.py` — pure orchestration: paginated backfill + incremental sync (`backfill` / `sync` retyped to `KlineClient`, so they drive Binance or OKX). `backfill_funding_rates` paginates full funding-rate history from `since_ms` (Binance-only, 1000/page), wired into `run_backfill` via `_sync_ancillary(funding_since_ms=)` so a deep OHLCV backfill now pulls matching funding depth (incremental `sync` keeps recent-only funding via `sync_funding_rates`). N3: `refresh_symbol_lifecycle(conn, client, symbols, now_ms=None)` — tracked set = existing lifecycle rows ∪ run symbols; present → status + `last_checked_ms` updated (`first_checked_ms` preserved); absent → `DELISTED` + sticky `delisted_noted_ms`; never deletes OHLCV/funding rows (forward-looking guard — pre-tracking delistings not enumerable from the free API)
- `analytics_runner.py` — thin wrapper: creates client, opens DB, calls sync lib. N3: `_refresh_lifecycle_safe` runs once at the top of `run_backfill`/`run_sync` (non-fatal — lifecycle must never block ingest); both runners wrap each symbol in try/except (log, continue, collect failures, `sys.exit(1)` at end with a summary) so a 25-symbol universe run survives single-symbol failures; re-runs are idempotent upserts
- `universe.py` — `DEFAULT_UNIVERSE_PATH = config/universe.toml`; `load_universe(path)` → stripped/uppercased/deduped symbol list; `FileNotFoundError` on missing file, `ValueError` on missing/empty/malformed `[universe].symbols`. CLI wiring: `buibui analytics backfill|sync --universe` (mutually exclusive with `--symbols`); `make universe-backfill` = `--universe --timeframes 1h 4h 1d 1w --since 2019-01-01`
