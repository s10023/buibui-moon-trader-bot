...
| **3** | Volume gate | Per-strategy opt-in | `volume_suppress`: drop low-vol signal (< 1.5× rolling mean). Directional variants (`_long`/`_short`, per-tf) supported. `SignalEvent.volume_spike` is still tagged for analysis but no longer gates behaviour (see `docs/audits/2026-05-20-volume-spike-boost-structural-inertness.md`). | `scanner.py` ~L740 | — |
| **4** | **Bias: regime gate (Step −1)** | **SOFT** | Drops signals whose strategy type is not in `enabled_regimes[type]` for the current 4h regime. Per-strategy overrides: `bos→[high_vol, range]` (PR #366 routing-audit flip), `fib_golden_zone→[range, high_vol]` (inverted from §6 default, PR #354). | `analytics/signal/gates.py::_apply_regime_gate` | §6 mapping was empirically wrong for `bos` (PR #351 replay verdict "DO NOT FLIP"); PR #366 flip + replay now reads "FLIP justified". Soft mode pending ≥2 weeks live evidence. |
| **4.5** | **Bias: direction filter (Step −0.5)** | **SOFT** | Per-strategy directional suppress — drops signals matching `[strategy_params.<name>].suppress_long` / `.suppress_short`. Cheapest filter; no HTF / regime data. Production: `bos.suppress_long = true` (T2c). | `analytics/signal/gates.py::_apply_direction_filter_gate` | Replay (PR #367): bos longs −0.27R / n=34,767 suppressed; shorts +0.14R / n=37,876 kept. Soft mode pending ≥2 weeks live evidence. |
...
| `tools/direction_filter_replay.py` | Join `backtest_trades` × per-strategy `suppress_long` / `suppress_short` flags to label trades suppressed/kept. | "FLIP justified" verdict for `bos.suppress_long`, PR #367 (n=72,643; suppressed −0.268R, kept +0.140R). |
